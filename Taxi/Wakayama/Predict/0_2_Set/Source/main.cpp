...
void myCreateInputDataFor0_3_Learn(MyData2 &myData2, std::vector<bool> &vIsValid, const std::string &fileName, const std::string &fileDire);
void myCreateInputDataFor0_3_Learn(MyData2 &myData2, std::vector<bool> &vIsValid, const std::string &fileName, const std::string &fileDire)
{
	// 保存path
	std::string fileRela = fileDire + "/" + fileName;
	...
	// add child elements
	{
		// indexToTrueIndex
		{
			...
			int trueIndex = 1;
			for (int i = 0; i < myData2.numCell; i++) {
...
int main()
{
	...
	// ここまでの営業領域に関する情報を保存
	{
		...
		// ./../Data/0_2_Set/Other/indexToTrueIndex.xml
		{
			// 設定値保存ファイル名
		...
		// ./../Data/0_2_Set/Other/trueIndexToIndex.xml
		{
			// 設定値保存ファイル名
	...
	// ./../Data/0_2_Set/Other/inputDataFor0_3_Learn.xml
	{
		// 保存ファイル名
		const std::string fileName = "inputDataFor0_3_Learn.xml";
		// ディレクトリのmakefileからの相対位置
		const std::string fileDire = "./../Data/0_2_Set/Other";
		myCreateInputDataFor0_3_Learn(myData2, vIsValid, fileName, fileDire);
	}
