...
struct MyData
{
	...
	// 2つの需要数カウントのサークルの中心点の最小距離[m]
	double minDistancebetweenDemandCountCircle;
...
MyData myReadMyData( const std::string &fileName, const std::string &fileDire )
{
	MyData ret;
	{
		...
		boost::property_tree::read_xml(fileRela, pt, boost::property_tree::xml_parser::no_comments);
		{
			...
			ret.demandCountCircleRadius = pt.get<double>( "myData.demandCountCircleRadius" );
			ret.minDistancebetweenDemandCountCircle = pt.get<double>( "myData.minDistancebetweenDemandCountCircle" );
			ret.vacantCountCircleRadius = pt.get<double>( "myData.vacantCountCircleRadius" );
		}
	}
	return ret;
}
...
void myDisplayMyData(const MyData &myData)
{
	...
	std::cout << cPutSp << "* demandCountCircleRadius" << "\n";
	{
		std::cout << cPutSp2 << "# ";
		std::cout << myData.demandCountCircleRadius;
		std::cout << "\n";
	}
	std::cout << cPutSp << "* minDistancebetweenDemandCountCircle" << "\n";
	{
		std::cout << cPutSp2 << "# ";
		std::cout << myData.minDistancebetweenDemandCountCircle;
		std::cout << "\n";
	}
	std::cout << cPutSp << "* vacantCountCircleRadius" << "\n";
	{
		std::cout << cPutSp2 << "# ";
		std::cout << myData.vacantCountCircleRadius;
		std::cout << "\n";
	}
}
...
void myDisplayCircleIndent(std::vector<std::vector<int>> &V);
void myDisplayCircleIndent(std::vector<std::vector<int>> &V)
{
...
void mySaveCircleIndent(std::vector<std::vector<int>> &V, const std::string &fileName, const std::string &fileDire);
void mySaveCircleIndent(std::vector<std::vector<int>> &V, const std::string &fileName, const std::string &fileDire)
{
		// 保存path
	std::string fileRela = fileDire + "/" + fileName;
	...
	// create the root element
	boost::property_tree::ptree& root = pt.put("v", "");
	...
	// add child elements
	{
		...
		{
			for (int i = 0; i < N; i++) {
				boost::property_tree::ptree& child = root.add("i", "");
				int M = V[i].size();
				for (int j = 0; j < M; j++) {
					boost::property_tree::ptree& cchild = child.add("i", "");
					cchild.add("j", V[i][j]);
				}
			}
		}
...
void myCreateInputDataFor1_Cron(MyData2 &myData2, std::vector<std::vector<int>> &vDemandCountCircleIndex, std::vector<std::vector<int>> &vVacantCountCircleIndex, std::vector<std::vector<int>> &vFlagSetCircleIndex, std::vector<bool> &vIsValid, const std::string &fileName, const std::string &fileDire);
void myCreateInputDataFor1_Cron(MyData2 &myData2, std::vector<std::vector<int>> &vDemandCountCircleIndex, std::vector<std::vector<int>> &vVacantCountCircleIndex, std::vector<std::vector<int>> &vFlagSetCircleIndex, std::vector<bool> &vIsValid, const std::string &fileName, const std::string &fileDire)
{
	// 保存path
	std::string fileRela = fileDire + "/" + fileName;
	...
	// add child elements
	{
		...
		{
			boost::property_tree::ptree& child = root.put("vDemandCountCircleIndex", "");
			int N = vDemandCountCircleIndex.size();
			{
				for (int i = 0; i < N; i++) {
					boost::property_tree::ptree& cchild = child.add("i", "");
					int M = vDemandCountCircleIndex[i].size();
					for (int j = 0; j < M; j++) {
						boost::property_tree::ptree& ccchild = cchild.add("i", "");
						ccchild.add("j", vDemandCountCircleIndex[i][j]);
					}
				}
			}
		}
		{
			boost::property_tree::ptree& child = root.put("vVacantCountCircleIndex", "");
			int N = vVacantCountCircleIndex.size();
			{
				for (int i = 0; i < N; i++) {
					boost::property_tree::ptree& cchild = child.add("i", "");
					int M = vVacantCountCircleIndex[i].size();
					for (int j = 0; j < M; j++) {
						boost::property_tree::ptree& ccchild = cchild.add("i", "");
						ccchild.add("j", vVacantCountCircleIndex[i][j]);
					}
				}
			}
		}
		{
			boost::property_tree::ptree& child = root.put("vFlagSetCircleIndex", "");
			int N = vFlagSetCircleIndex.size();
			{
				for (int i = 0; i < N; i++) {
					boost::property_tree::ptree& cchild = child.add("i", "");
					int M = vFlagSetCircleIndex[i].size();
					for (int j = 0; j < M; j++) {
						boost::property_tree::ptree& ccchild = cchild.add("i", "");
						ccchild.add("j", vFlagSetCircleIndex[i][j]);
					}
				}
			}
		}
...
int main()
{
	...
	// 需要数・空車数カウントのサークルとフラグサークルを作成する
	std::vector<std::vector<int>> vDemandCountCircleIndex(myData2.numCell);
	std::vector<std::vector<int>> vVacantCountCircleIndex(myData2.numCell);
	std::vector<std::vector<int>> vFlagSetCircleIndex(myData2.numCell);
	{
		std::cout << "需要数・空車数カウントのサークルとフラグサークルを作成中..." << "\n";
		int N = vIndex.size();
        boost::progress_display show_progress( N );
		double maxCircleLength = std::max({myData.demandCountCircleRadius, myData.vacantCountCircleRadius, myData.minDistancebetweenDemandCountCircle});
		for (int i = 0; i < N; i++) {
			// 処理
			{
				if (vIsValid[i]) {
					...
					{
						{
							for (int j = 0; j < N; j++) {
								...
								bool isWithinNeighborhood = (pairHoge.tStation.getLength() <= maxCircleLength);
								bool isValidCell = vIsValid[j];
					...
					// vDemandCountCircleIndex[i]
					// vVacantCountCircleIndex[i]
					// vFlagSetCircleIndex[i]
					// に対して登録する個数を求める．
					int N1 = 0;
					int N2 = 0;
					int N3 = 0;
					{
						int NHoge = vPairIndexTSTation.size();
						for (int j = 0; j < NHoge; j++) {
							double hoge = vPairIndexTSTation[j].tStation.getLength();
							if ( hoge <= myData.demandCountCircleRadius) {
								N1++;
							}
							if (hoge <= myData.vacantCountCircleRadius) {
								N2++;
							}
							if (hoge <= myData.minDistancebetweenDemandCountCircle) {
								N3++;
							}
						}
					}
					// 容量の確保
					{
						vDemandCountCircleIndex[i].reserve(N1 + 1);
						vVacantCountCircleIndex[i].reserve(N2 + 1);
						vFlagSetCircleIndex[i].reserve(N3 + 1);
					}
					// 第一要素は-1(invalidのセルもあるため)
					{
						vDemandCountCircleIndex[i].push_back(-1);
						vVacantCountCircleIndex[i].push_back(-1);
						vFlagSetCircleIndex[i].push_back(-1);
					}
					// 第二要素以降を登録していく
					{
						for (int j = 0; j < N1; j++) {
							int hoge = vPairIndexTSTation[j].index;
							vDemandCountCircleIndex[i].push_back(hoge);
						}
						for (int j = 0; j < N2; j++) {
							int hoge = vPairIndexTSTation[j].index;
							vVacantCountCircleIndex[i].push_back(hoge);
						}
						for (int j = 0; j < N3; j++) {
							int hoge = vPairIndexTSTation[j].index;
							vFlagSetCircleIndex[i].push_back(hoge);
						}
					}
				}else{
					// 第一要素は-1(invalidのセルもあるため)
					vDemandCountCircleIndex[i].reserve(1);
					vDemandCountCircleIndex[i].push_back(-1);
					vVacantCountCircleIndex[i].reserve(1);
					vVacantCountCircleIndex[i].push_back(-1);
					vFlagSetCircleIndex[i].reserve(1);
					vFlagSetCircleIndex[i].push_back(-1);
				}
			}
			// boost::this_thread::sleep(boost::posix_time::milliseconds(1));
	...
	// // 表示
	// myDisplayCircleIndent(vDemandCountCircleIndex);

	// // 表示
	// myDisplayCircleIndent(vVacantCountCircleIndex);

	// // 表示
	// myDisplayCircleIndent(vFlagSetCircleIndex);

	std::cout << "需要数・空車数カウントのサークルとフラグサークルを保存中..." << "\n";
	{
		{
			// 保存ファイル名
			const std::string fileName = "vDemandCountCircleIndex.xml";
			// ディレクトリのmakefileからの相対位置
			const std::string fileDire = "./../Data/0_2_Set/Other";
			mySaveCircleIndent(vDemandCountCircleIndex, fileName, fileDire);
		}
		{
			// 保存ファイル名
			const std::string fileName = "vVacantCountCircleIndex.xml";
			// ディレクトリのmakefileからの相対位置
			const std::string fileDire = "./../Data/0_2_Set/Other";
			mySaveCircleIndent(vVacantCountCircleIndex, fileName, fileDire);
		}
		{
			// 保存ファイル名
			const std::string fileName = "vFlagSetCircleIndex.xml";
			// ディレクトリのmakefileからの相対位置
			const std::string fileDire = "./../Data/0_2_Set/Other";
			mySaveCircleIndent(vFlagSetCircleIndex, fileName, fileDire);
		}
	}
	...
	// ./../Data/0_2_Set/Other/emptyCircle.xml
	{
		// 保存ファイル名
		const std::string fileName = "emptyCircle.xml";
		// ディレクトリのmakefileからの相対位置
		const std::string fileDire = "./../Data/0_2_Set/Other";
		myCreateEmptyCircle(fileName, fileDire);
	}
	// ./../Data/0_2_Set/Other/inputDataFor1_Cron.xml
	{
		...
		// ディレクトリのmakefileからの相対位置
		const std::string fileDire = "./../Data/0_2_Set/Other";
		myCreateInputDataFor1_Cron(myData2, vDemandCountCircleIndex, vVacantCountCircleIndex, vFlagSetCircleIndex, vIsValid, fileName, fileDire);
	}
